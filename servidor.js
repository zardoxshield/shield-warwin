...
function criarProjeto(nome, tipo) {
  ...
  Object.entries(template).forEach(([arquivo, conteudo]) => fs.writeFileSync(path.join(dir, arquivo), conteudo));
...
http.createServer((req, res) => {
  ...
  if (pathname === '/publicar') {
    ...
    const request = https.request(options, () => res.end('Publicado com sucesso!'));
